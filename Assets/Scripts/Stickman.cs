...
public class Stickman: MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (disableL == false) 
        {
            ...
            Transform child = lArm.transform.GetChild(0);
            if(child.gameObject.tag == "Gun")
            {
                lArm.transform.GetChild(0).rotation = Quaternion.Euler(0.0f,0.0f,-angle + 180);
        ...
        if (disableR == false)
        {
            ...
            // add other tags for other guns
            Transform child = rArm.transform.GetChild(0);
            Debug.Log(child.gameObject.tag);
            if(child.gameObject.tag == "Gun")
            {
                Debug.Log("idontlikeyou");
