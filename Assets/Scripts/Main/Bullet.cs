...
public class Bullet : MonoBehaviour {
    ...
    public float speedLimitToHit;
    public float lastFleshHitTime = 0f;
    ...
    void OnCollisionEnter2D(Collision2D coll)
    {
        ...
        else if(((coll.gameObject.tag == "lArm" || coll.gameObject.tag == "rArm" || coll.gameObject.tag == "rLeg" || coll.gameObject.tag == "lLeg" || coll.gameObject.tag == "lFoot" || coll.gameObject.tag == "rFoot") && active)&& coll.relativeVelocity.magnitude > speedLimitToHit)
        {
            if((Time.time - lastFleshHitTime) > 0.5f)
            {
                lastFleshHitTime = Time.time;
                stick.PlayAudio(fleshHitAudio, gameObject);
            }
            ...
            // so yeah the blood shit works now
            ...
            // also remember health and shit, cause rn he just dies on hit
            ...
            if(coll.gameObject.transform.parent != null)
            {
                ...
                hitStick.health -= (coll.relativeVelocity.magnitude * hitMultiplier );
                if(hitStick.health <= 0 && !hitStick.dead)
                {
                    hitStick.dead = true;
                    stick.PlayRandomClip(stick.dyingAudio, hitStick.Head.transform.position);
                    if(!screenF.recurse)
                    {
                        screenF.Activate();
                    }
        ...
        else if(((coll.gameObject.tag == "Body" || coll.gameObject.tag == "Head") && active) && coll.relativeVelocity.magnitude >  speedLimitToHit)
        {
            //Stickman hitStick = coll.gameObject.transform.root.gameObject.GetComponent<Stickman>();
            if((Time.time - lastFleshHitTime) > 0.5f)
            {
            ...
            if(coll.gameObject.tag == "Head")
            {
                ...
                stick.PlayRandomClip(stick.headShotAudio, hitStick.Head.transform.position);
                hitStick.dead = true;
            ...
            {
                hitStick.health -= (coll.relativeVelocity.magnitude * hitMultiplier * 3);
                if(hitStick.health <= 0 && !hitStick.dead)
                {
                    stick.PlayRandomClip(stick.dyingAudio, hitStick.Head.transform.position);
                    hitStick.dead = true;
        ...
        else if(coll.gameObject.tag != "Gun" && manager.canPlay && coll.relativeVelocity.magnitude > 3f)
        {
            //Destroy(gameObject);
        

            if (coll.gameObject.tag != "Background" && coll.gameObject.tag != gameObject.tag)
            {
                ...
                //{
                    ...
                    // }
                    //stick.Start();
                    // these sounds are fucking shit so i turned them off
                    if(stick.Audios.ContainsKey(coll.gameObject.tag))
                    {
                        //int key_value = Random.Range(0, stick.Audio_Map.Count - 1);//stick.Audio_Map[fileName];
                        AudioSource[] audi = stick.Audios[coll.gameObject.tag];
                        stick.PlayRandomClip(audi, gameObject.transform.position);
                    ...
                    else
                    {
                        ...
                        stick.PlayRandomClip(audi, gameObject.transform.position, coll.relativeVelocity.magnitude / 10.0f);
                        //audi[key_value].volume = coll.relativeVelocity.magnitude / 10.0f;
                        // audi[key_value].PlayOneShot(audi[key_value].clip);
                        manager.lastPlayTime = Time.time;
                    }
