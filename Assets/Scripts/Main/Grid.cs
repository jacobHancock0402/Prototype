    ...
    void Start()
    {
        ...
        for(int x=0;x<numNodesX;x++)
        {
            for(int y=0;y<numNodesY;y++)
            {
                ...
                //filter.NoFilter();
                colls = Physics2D.OverlapCircleAll(position, nodeRadius); 
                bool walkable2 = true;
                Collider2D walkable = new Collider2D();
        ...
        for(int x1=1;x1<numNodesX - 1;x1++)
        {
            for(int y1=1;y1<numNodesY - 1;y1++)
            {
                ...
                // shouldn't this be node diameter?this why have to *2
                int yminus1 = y1-1;
                int yplus1 = y1+1;
                int xplus1 = x1+1;
                int xminus1 = x1-1;
                // this shit weird man, no work for - 1
                if(!grid[x1,y1].Solid)
                {
               ...
               // {
                ...
                foreach(Node Neighbour in Neighbours)
                {
                    if(!compareNodeToList(Neighbour, closedList) && ((Neighbour.WalkAble && currentNode.WalkAble && !Neighbour.ClimbAble) || (currentNode.ClimbAble && Neighbour.ClimbAble && currentNode.y <= Neighbour.y) ||(currentNode.ClimbAble && (Neighbour.WalkAble && !Neighbour.ClimbAble)) || ((currentNode.WalkAble && !currentNode.ClimbAble) && Neighbour.ClimbAble)  || (!Neighbour.Solid && Neighbour.y < currentNode.y && Neighbour.x == currentNode.x)))
                    {
                        ...
                        // is it on comparison on closed and open list?, what else could it be the fuck?
...
void OnDrawGizmosSelected()
     {
        //Gizmos.DrawWireCube(transform.position, new Vector3(gridSizeX, 1, gridSizeY));
            Debug.LogError("songcalledyell");
            //f(grid != null)
            //{
        //Node PlayerNode = GetCurrentNode(Player.position);
        Debug.LogError("tyler");
        ...
        Debug.LogError("songcalledyellow");
               ...
               //  }
               if(n.WalkAble && !n.onPath && n.Incline )
               {
                    Gizmos.color = Color.green;
                    Debug.LogError("wanton");
                    // idk what's going, dude isn't moving after messing with grid, so tried to look at it with thes
                    // though it can't find path, so then nulls and can't reach crouch part of gun searhh
                    // but can't see grid even though it enters the branch below
               }
                else
               {
                Debug.LogError("swansong");
                    Gizmos.color = Color.red;
               }
               Gizmos.DrawSphere(n.position, nodeRadius);
                 Gizmos.DrawCube(n.position, Vector3.one * (nodeRadius * 2));
                 Debug.LogError("runningfromtheband");
           }
